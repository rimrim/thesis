...
\begin{figure}[htbp!] 
\centering \procedure{Benhamouda Protocol for LWE}{
  \textbf{Prover} \> \> \textbf{Verifier}\\
...
ZKP technique with Schnorr-based one and save us many rounds of communication
due to the soundness error decreasing from $2/3$ to $1/n$. However, the new
proof technique could not prove that the encrypted message was in binary format:
The proof only shown the noises are small ($\mathbf{s}_{e} \leq 2\beta$) and the
message is small ($\mathbf{s}_{m} \leq 2t$). To solve this problem, we include
one homomorphic operation after the $Client$ was able to pass the proof. The operation works as follows.
...
As described before in previously, the server garbles
