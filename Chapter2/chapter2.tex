...
many cryptographic application: Outsourcing storage and computation, Private
Information Retrieval, Multiparties Computation, etc . The idea was introduced
since late 1970s \cite{rivest1978data} and has been actively researched lately
...
\cite{homenc}. This section introduce the concept and discuss some properties
and connections with secure computation protocols.
...
and ciphertexts. We refer to the ciphertexts output from the \(Encrypt\)
operation as ``fresh ciphertext'' and outputs from \(Evaluate\) as ``evaluated
ciphertext''. There are three circuit families related to \textit{homomorphic capacity } of \(\mathbb{E}\):
\begin{description}
\item[Fully Homomorphic Encryption] We say that \(\mathbb{E}\) is fully
  homomorphic if it is correct for all boolean circuit. In this case we can
...
\item[Additive Homomorphic Encryption] We say that \(\mathbb{E}\) is additively
  homomorphic if it is correct for families of circuit made up of only XOR
...
practical enough. In this project, we focus our work on Somewhat Homomorphic
Encryption (SHE) systems: the BV system by \cite{brakerski2011fully} and the BGV
system \cite{brakerski2014leveled}, they allows additions and some levels of
multiplications on the ciphertexts, and they serves well our purpose in the
privacy preserving authentication context. The security of these cryptosystems
is based on the hardness of the Ring-Learning With Error (RLWE) problem
\cite{lyubashevsky2010ideal}. This section discuss briefly such systems.
\begin{description}
...
system (Stern-ZKP) that serve as the basic tool in the later constructions of
the thesis.

Traditionally, proofs are are static objects that can be expressed in written
form so that they can be verified later. However, after one verifies the
validity of a statement, he learns all the information about the proof, this is
sometimes undesirable. Interactive proof works differently. It is an interative
protocol between two parties: a prover and a verifier. The protocol involves
messages exchange between them and at the end, the verifier outputs a bit
``Accept'' if he is convinced that the statement is true, otherwise he output
``Reject''. We focus on a specific category of ZKP: \(\sum\)-protocol. There are
generally 3 messages exchanged in one round of such protocol. The prover first
...
verifier can check the validity of the statement using a function of the
messages received. In each round, a dishonest prover may guess the verifier's
challenge to prepare the proper response with some constant
probability. However, the protocol can be repeated many rounds to reduce the
probability of guessing correctly all the challenges to negligibly small.  More
...
There are several types of ZKP, which are the building blocks in many
