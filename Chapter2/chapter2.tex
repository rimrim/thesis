...
\item[Quantum Resistance.] In 1994, Shor \cite{shor1994algorithms} showed that
  ...
  idea is feasible in near future. If this happens, it will have significant
...
When running LLL experimentally (\cite{nguyen2006lll}) on a random lattice, it will become apparent that it performs much better than the bound showed in Theorem \ref{theo:LLLHF}. Specifically, $HF$
...
shortest vector problem. Using LWE as the basis for quantum resistant cryptographic techniques  therefore deserves high confidence. In practice, the best known
attack against LWE amounts to reducing the problem to the SIS problem: Given a LWE
...
\subsection{Symmetric cryptosystems from LWE}
\label{sec:LWESymmetric}
To randomize cryptosystem based on LWE:
\begin{description}
...
\item [Security.] This section discusses how to analyze the security of
  ...
  \item [Security Reduction from IND-CPA to DLWE.] Given the formal security
    game, we need to prove that the defined cryptosystem satisfies such a
    ...
    Suppose there was an efficient IND-CPA attack algorithm $B$, breaking the
...
\item [Security.] Similarly to the symmetric key system, the
  ...
  \item [Why does $D$ work.] Consider two $LWE$ scenarios for $\vec{y}$:
    ...
    \item If the distribution P of
      ...
      of $b$ and the public key $\vec{y}$, which contains no information on $b$,
    ...
    \item By LHL,
      ...
      \end{theorem}
    \item For future reference on parameter choices, we consider the following example of
      parameter value choice for Regev's cryptosystem. We can rearrange the
      requirements for the LHL condition to hold, it tells us how large $m$ should
      be:
      \[
...
\item[NTRUEEncrypt \cite{hoffstein1998ntru}] Both the public-key and the
  ...
  % \(R^{-} = \mathbb{Z}[x]/(x^{n} -1 \) ) instead of modulo $x^n + 1$. We briefly describe the scheme
  % as follows.
  % \begin{description}
...
The syntax in the definition can be extended naturally to vectors of plaintexts
...
\item[The Ring Learning With Errors Problem.]
  \begin{definition}
    ...
    $(\mathbf{a_i},\mathbf{b_i})$ are generated by sampling
    $\mathbf{a_i} \randomsample R_q$, and finally 
    $\mathbf{b_i} = \mathbf{a_i}.\mathbf{s} + \mathbf{e_i}$ are computed.
  \end{definition}
...
\begin{definition}
  [Interactive Proof System] An interactive proof system for a language
...
An interactive proof is said to be \(zero-knowledge \) if after reading the
...
\begin{definition}
  [Statistical ZKP] An interactive proof system \(\Angle{P,V}\) for a language
...
There are several types of ZKP, which are the building blocks of many
...
correctness and statistical zero-knowledge proofs. As per their
results, each round of communication costs $\log\beta\tilde{O}(n \log Q)$ bits,
...
\begin{description}
\item[Setup.] Let COM be a statistical hiding and computational biding commitment scheme (\cite{kawachi2008concurrently} show that such scheme can be constructed based on the hardness of ISIS problem).  Before the interaction, P and V
...
items to him, he is unaware of which one has been requested
...
authentication scheme. We then present different variants of privacy preserving authentication protocol according to the
models.
...
We first describe the protocol and its security model generically.  We then go
into details and further use it as a framework in our specific proposal. To
formulate precisely the definition of security, we will describe the protocols'
security models in terms of \textbf{attack games} played bettwen two parties:
the \textbf{challenger} and an \textbf{adversary}. Generally, the challenger
\(\challenger\) follows a simple, fixed protocol and the adversary
...
at the end of the games, \(\attacker\) outputs some value. The attack games
define a probability space based on this output value, and it defines the
adversary's \textit{advantage}, which normally measures the difference between
the probabilities of two events in this probability space: One is the event that
the adversary win the \textit{real} game, the other one is the event that the
adversary win the game with one (or some) of the system's components replaced by
an idealized version of such component(s).

\subsubsection{Desirabtele properties}
...
\item[Algorithms and Procedures in privacy-preserving settings:] We describe the
  high-level constructions of the protocol as follows:
  \begin{description}
