...
conclude that there must not exist any efficient algorithm to find a collision for
the hash function.
\begin{theorem}
...
\begin{itemize}
\item How should we choose the parameter values to obtain a given security level?
...
\begin{definition}
  ...
  \label{def:GSO}
\end{definition}
GSO is basically a way to convert a basis a given non-orthogonal basis into a set of orthogonal vectors. Figure \ref{fig:gso} shows an
...
reduced basis, and we take the shortest vector in that result basis. Our aim is to discover the difference between the length of this vector and the shortest vector in the lattice.
\begin{theorem}
...
practice we usually use HF as a main measure. Again, LLL produces an HF that is
...
the dimension of the lattice. When the dimension is large, the approximation factor will grow
...
on the output that LLL can give. Generally, the output vectors tend to be shorter than
...
(Fincke-Phost/Kannan) does it in time $2^{O(n\log(n))}$, it performs even worse than
