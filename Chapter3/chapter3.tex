...
\item[Malicious Client Model Impact] It is important for the protocol to be secure
  ...
  protocol transcript but also tries to draw information (this model is also known
...
\item[Setup.] $\user$ and $\server$ initialize the parameters, taking into account the following categories:
  ...
  \item[Ring-LWE based techniques parameters.] These parameters are used in the lattice-based cryptosystem which provides
    ...
    \item $n$: Integer $n$ defining the plaintext and ciphertext ring spaces. This will be refered to as the degree of
...
of these events, bound by $Pr[bad_0] \leq q\varepsilon_{ZK1}$. In other words, the advantage of $\attacker$ in
\textit{game 1} is
\[
