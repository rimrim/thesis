...
In the previous chapter, we assumed that given the Hamming Distance (HD) of two
binary bitstring templates, it is infeasible to infer or learn any information
about the original templates. The assumption is plausible as long as the
distributions of the registered and the queried templates are not in any way
correlated, studying about those distributions is out of scope of the
project. However, in this chapter, we introduce our first attempt to hide this
information from the server as well. Such result not only helps the protocol
working regardless of such dependence, it is also the first step we can make to
improve the authentication protocol to be secured against an \textit{active }
server security model. Although the communication size of this variant might not
be suitable to apply in practice for current network infrastructures, one can
still find it helpful when using some generic techniques of the protocol when
applying them to balance the computation time and the communication size of
lattice-based cryptographic protocols. The contributions of this variant includes
\begin{itemize}
...
\item Some extensions of the Zero Knowledge Proof (ZKP) technique that we used
  in previous chapter to prove also the formats of a message in addition to the
  knowledge of the plaintext itself.
\item The combination of ZKP protocols to balance the communication size
  tradeoffs when used with lattice-based cryptosystem.
\end{itemize}
...
AND-composition, OR-composition. The BV-ZKP (Figure
\ref{fig:belhamoudaProtocol}) protocol is used to demonstrate how to construct
advanced $\sum-protocol$.
\begin{description}
\item[AND-COMPOSITION] Given two relations $R_1 = \left\{ (v_1,w_1) \right\}$
  ...
  (Figure \ref{fig:belhamoudaProtocolAND})

\item[OR-Compostion] In this composition, given the $\sum-protocol$ for 2
  relations $R_{1}$ and $R_{2}$, the goal is to construct a protocol for the
  relation
  $R_{1} \vee R_{2} = \{ (v_1, v_2; w_1, w_2):(v_1;w_1) \in R_1 \vee (v_2;w_2)
  \in R_2\} $. The main idea to construct this composition is that the verifier
  can let the prover using the simulator of the \(\sum-protocol\) for one of the
  relation \(R_{i}\) that the prover does not know the witness
  \((i = 1 \text{ or } i = 2)\). The verifier provides a single challenge \(c\)
  such that the prover can split that into two challenges \(c_{1}, c_{2}\)
  provided that \(c_{1}, c_{2}\) satisfy a liner constraint in terms of \(c\),
  for example \(c = c_{1} \xor c_{2}\). The final OR-combination proof is
  obtained by composing one run of the \(\sum-protocol\) with another run of the
  simulator for the \(\sum-protocol\).
\end{description}
