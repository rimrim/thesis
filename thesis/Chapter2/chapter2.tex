...
There are other interesting advantages, one of them is provable security, in
...
\begin{description}
    ...
    \item[Lattices]
        \begin{description}
            \item[Definition] An n-dimensional (full-rank) \emph{lattice} $L(B)$ is the
                set of all integer linear combinations of some \emph{basis} set
                of linearly independent vectors $\vec{b_1},\dots,\vec{b_n} \in
                \mR^n$:
                \[
                    L(B) = \left\{ c_1 \vec{b_1} + c_2 \vec{b_2} + \dots + c_n
                    \vec{b_n} : c_i \in \mZ, i = 1, \dots, n \right\}
                \]
                The $ n \times n$ matrix $B = (\vec{b_1},\dots, \vec{b_n})$ is
                called a basis for $L(B)$. There are infinitely many different
                bases for a lattice.
            \item[Example] Consider a 2-dimensional lattice generated by the basis
                $(\vec{b1}, \vec{b2}) = \left( \begin{bmatrix}
                        1 \\ 2
            ...
            \begin{figure}[h]
                ...
                \caption{Example of a 2-dim lattice}
                \label{fig:2dimLattice}
            \end{figure}
            , to generate a lattice from a basis, we compute all the
            integer multiples of each vector of the basis. Then we add them
            together to generate new vectors. The result is an infinite grid of
            vectors, this grid is a lattice. We can illustrate two or
            three dimensional lattice easily, for higher dimensions, it is not
            possible to draw the lattice, but the intuition stays the same and
            all the algebraic operations can be done easily on the coordinates
            of the vectors. In lattice-based crypto, we work with high dimension
            lattices ($n > 100$) for security properties. In fact, we can also
            say that a lattice is an infinite group
            (a group is a mathematical structure,
            a set with one operation on it.) with the main operation is
            addition.
        \item [Fundamental paralellepiped] For an n-dimensional lattice basis
            $B = \left( \vec{b_1}, \dots, \vec{b_n} \right) \in \mR^{n \times
            n}$, the \emph{fundamental paralellepiped} (FP), denoted $P(B)$, is the
            set of all real valued $[0,1)$-linear combinations of some basis set
            of linearly independent vectors $\vec{b_1}, \dots ,\vec{b_n} \in
            \mR^n$.
            \begin{figure}[h]
                \centering
...
\begin{description}
    \item[Ajtai's Random q-ary 'perp' lattices] Given an integer \emph{q} and a
        ...
        average-case} security reduction of Ajtai: \todo{rephrase this},
    ...
    \item [SIS problem (Small Integer Problem)] Informally, SIS is just the name given to
        ...
        \begin{definition}
            ...
            \label{}
        \end{definition}
        \todo[inline]{Notation for random sampling $A \randomsample \mathbb{Z}$}
    ...
    \item [A cryptographic application from SIS problem.] We discuss first a
        ...
        \begin{definition}
            [Ajtai's Hash Function.] Pick $A=(\mathbf{a_{i,j}}) \randomsample
...
lattice-based systems (by access the security of underlying problems such as SVP
    ...
    We first recall this important concept related to lattices:
    ...
    2-dimension \todo[inline]{redo figure in sage}
    \begin{figure}[h]
        \centering
    ...
    \end{figure}
    We can intepret the second relation as a rotated coordinates system. Another nice
    ...
    \begin{description}
        \item[LLL algorithm] LLL not only finds short vectors but also
            ...
            \begin{theorem}
                [LLL run time] The number of iterations of LLL on an input basis
        ...
        \item [How can LLL is used to solve $\gamma-SVP$]. The question is how
            ...
            lattice.
            \begin{theorem}
                [Short vector from LLL]. The LLL algorithm solves in polynomial
        ...
        \item [Ajtai's hardness proof for SIS] We have looked at the best
            ...
            \begin{theorem}
                [Ajtai's hardness proof for SIS.] If there is an algorithm
