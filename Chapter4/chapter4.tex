...
The proof of Theorem \ref{theo:server} can be gained using a sequence of games
...
transcripts using the secret witnesses, we simulate them using the statistical
...
The proof of theorem \ref{theo:client} can be obtained using a sequence of games
between the challenger $\challenger$ and the adversary $\attacker$. We here
present such a sequence, as well as the relations among them, to demonstrate
first the type I security model proof.
  ...
  \fbox{
   \begin{gameproof}[nr=3]
     \gameprocedure[linenumbering, mode=text]{
       ...
       {$proverView1 \gets \mathbf{ZKPSimulator}()$}\\
       $\enc{\mathbf{HD}_{i}} = EvalDistance(\enc{\mathbf{X}_{k}},\enc{\mathbf{Y}_{i}})$\\
       \gamechange{$r_{i} \gets enc$ ,$\0{\mathbf{HD'}_{i}} = Add(\enc{\mathbf{HD}_{i}},\enc{r_{i}})$}\\
       $HD'_{i} \gets Dec(\enc{\mathbf{HD'}_{i}},\sk)$\\
       {$proverView2 \gets \mathbf{ZKPSimulator}()$}\\
       $HD_{i} \gets HD'_{i} - r_{i}$\\
       {$res_{i} \gets Verify(X_{k},Y_{i})$}\\
...
\ref{le:hdcorrectness} and \ref{le:Renyi} and \cite{naehrig2011can}, we can take
\(\sigma = 50\) to make the protocol secure against the lattice attacks
(\cite{micciancio2008lattice}) and also satisfy the circuit privacy
requirement. We can take \(t = 2048\) to define the plaintext space ring
\(R_{t}\), this is enough to cover Hamming Distance of bit strings up to 2048
bits. We need \(n \geq 2048\) for the packing ciphertext. With these parameters,
we need \(q\) to be approximately 70 bits to satisfy
\(16n^{2}t^{2}\sigma^{4} < q\).  The proposed scheme works with only 1 level of
homomorphic multiplication for the proposed set of parameters
$(n = 2048, q \approx 2^{70}, t = 2048, \sigma = 50)$, proof of concept
