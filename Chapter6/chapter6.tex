...
interest as building blocks for other secure computation protocols based on the
BV SWHE scheme:
\begin{itemize}
\item A ZKP technique to prove the correctness of re-encryption of a
  ...
  addition/multiplication arithmetic operation) as a unary-encoded plaintext
...
communication overhead: we replace the Stern-based ZKP technique which require
many rounds of proof for security with Schnoor-based technique (only 1 round
required). Other cryptographic tools including Garbled Circuit and Oblivious
Transfer will be used in the HD comparision step to avoid another proof during
authentication.
